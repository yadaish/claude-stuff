...
- **Do not add comments that merely restate what the code already makes
  obvious.** If a reader can understand the intent from the code itself, the
  comment is noise — omit it. Add a comment only when it carries information the
  code cannot: a genuine gotcha, a non-obvious reason, a known problem, or a
  temporary workaround (mark those clearly, e.g. `# TODO`/`# HACK`, and say why).
...
- No comments that just restate obvious code; comments only for gotchas,
  reasons, known issues, or temporary workarounds.
